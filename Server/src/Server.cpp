            ...
            if (event.type == ENET_EVENT_TYPE_CONNECT) {
                ...
                if (id_Client.size() == 0) {
                    ...
                    for (currentPeer = server -> peers; currentPeer < & server -> peers [server -> peerCount]; ++ currentPeer) {
                        if (currentPeer -> state != ENET_PEER_STATE_CONNECTED)
                            continue;
                        std::string message_Send = "ID=" + std::to_string(numberC);
                        ENetPacket *packet = enet_packet_create(message_Send.c_str(), message_Send.length(), ENET_PACKET_FLAG_RELIABLE);
                        enet_peer_send(currentPeer, 0, packet);
                    }
                }
                    ...
                    if (check == true) {
                        ...
                        for (currentPeer = server -> peers; currentPeer < & server -> peers [server -> peerCount]; ++ currentPeer) {
                            if (currentPeer -> state != ENET_PEER_STATE_CONNECTED)
                                continue;
                            std::string message_Send = "ID=" + std::to_string(itr->first);
                            ENetPacket *packet = enet_packet_create(message_Send.c_str(), message_Send.length(), ENET_PACKET_FLAG_RELIABLE);
                            enet_peer_send(currentPeer, 0, packet);
                        }
                    }
                        ...
                        // std::string message_Send = "ID=" + std::to_string(id_Client.end()->first);
                        for (currentPeer = server -> peers; currentPeer < & server -> peers [server -> peerCount]; ++ currentPeer) {
                            if (currentPeer -> state != ENET_PEER_STATE_CONNECTED)
                                continue;
                            std::string message_Send = "ID=" + std::to_string(id_Client.end()->first);
